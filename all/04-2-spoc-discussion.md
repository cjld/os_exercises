...
> 下面对访存次数n做归纳，假设LRU(i)为物理内存页面数为i时的缺页次数，其中i = 1,2,3...

> 当n = 1时，LRU(i+1) = LRU(i) = 1;

> 当n = p时，假设LRU(i+1) <= LRU(i)。那么当n = p + 1时，对于前面p次访存，由归纳假设LRU(i+1) <= LRU(i)。 考虑第p+1次访存，只会出现三种情况，物理内存页面数为i时发生缺页，页面数为i+1时不发生缺页；两者都发生缺页；两者都不发生缺页。 由于最近访问的i个页面，一定包含于最近访问的i+1个页面，所以不会出现i+1出现缺页而i不出现缺页。 所以当n = p + 1时，LRU(i+1) <= LRU(i)。
...
> 学号为2012011360，实现的是改进的clock 页置换算法
